...
LineFitWidget::LineFitWidget(QWidget *parent) : QWidget(parent)
{
    ...
    m_GN  = new QCheckBox(QString("高斯牛顿"));
    m_LM = new QCheckBox(QString("LM"));
    ...
    buttonGroup->addButton(m_GN);
    buttonGroup->addButton(m_LM);
...
QList<double> LineFitWidget::get_Line(QList<QPointF> listP)
{
    ...
    if(m_OLS->isChecked())
    {
        vector_x = GlobleAlgorithm::getInstance()->LeastSquares(matA,matB);
    }
    //勾选迭代加权最小二乘
    else if(m_IRLS->isChecked())
    {
        ...
        vector_x =GlobleAlgorithm::getInstance()->IterativeReweightedLeastSquares(matA,matB,p,iteCount);
    }
    //勾选高斯牛顿法
    else if(m_GN->isChecked())
    {
        int iteCount= m_iterationCount->value();
        //初始参数为1
