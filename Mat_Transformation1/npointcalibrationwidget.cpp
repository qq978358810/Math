...
NPointCalibrationWidget::NPointCalibrationWidget(QWidget *parent) : QWidget(parent)
{
    ...
    GLayout_1->addWidget(m_textEdit,1,0);
...
void NPointCalibrationWidget::slot_executeClick()
{
    ...
    //执行标定，获取结果
    _mat mat = GlobleAlgorithm::getInstance()->calibration(coordL);


    //显示标定结果
    m_textEdit->append(QString("内部算法【1】标定: 矩阵结果:"));
    m_textEdit->append(QString::number(mat.a,'f',15)+" , "+QString::number(mat.b,'f',15)+" , "+QString::number(mat.c,'f',15));
    m_textEdit->append(QString::number(mat.d,'f',15)+" , "+QString::number(mat.e,'f',15)+" , "+QString::number(mat.f,'f',15));
    m_textEdit->append(QString::number(mat.g,'f',15)+" , "+QString::number(mat.h,'f',15)+" , "+QString::number(mat.i,'f',15));
    m_textEdit->append("\r\n");
    ...
    if(m_OLS->isChecked())
    {
        abc_3_1 = GlobleAlgorithm::getInstance()->LeastSquares(matA,matB_x);
        abc_3_2 = GlobleAlgorithm::getInstance()->LeastSquares(matA,matB_y);
    }
    //勾选迭代加权最小二乘
    else if(m_IRLS->isChecked())
    {
        ...
        abc_3_1 =GlobleAlgorithm::getInstance()->IterativeReweightedLeastSquares(matA,matB_x,p,kk);
        abc_3_2 =GlobleAlgorithm::getInstance()->IterativeReweightedLeastSquares(matA,matB_y,p,kk);
    }
    else
    {
        return ;
    ...
    m_textEdit->append(QString::number(abc_3_1(0,0),'f',15)+" , "+QString::number(abc_3_1(1,0),'f',15)+" , "+QString::number(abc_3_1(2,0),'f',15));
    m_textEdit->append(QString::number(abc_3_2(0,0),'f',15)+" , "+QString::number(abc_3_2(1,0),'f',15)+" , "+QString::number(abc_3_2(2,0),'f',15));
    m_textEdit->append(QString::number(0,'f',15)+" , "+QString::number(0,'f',15)+" , "+QString::number(1,'f',15));
    m_textEdit->append("\r\n");
}
//打开文件-》从文件读取坐标
